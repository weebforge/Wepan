...
import { WeebPanelPermissionResolvable, WeebPanelPermissionsBitField } from "./WepanPermission"
...
export function DefaultAccessKeyGenerator(userId: string, token: string, permissions: number, seed: BigInt): bigint {
  const data = [seed, userId, token, permissions].join("|")

  return BigInt(`0x${nodeCrypto.createHash("sha256").update(data).digest("hex").slice(0, 40)}`)
}
...
export class WeebPanel extends ForgeExtension {
  ...
  #generatedAccessKeys: Map<string, { userId: string; token: string; permissions: number }> = new Map()
  ...
  constructor(options: IWeebPanelOptions) {
    ...
    this.options = {
      ...
      accessKeyGeneratorSeed: options.accessKeyGeneratorSeed ?? DefaultAccessKeyGeneratorSeed,
    }

    if (this.options.access.length === 0)
      Logger.warn("WeebPanel: No access keys provided. The panel will be inaccessible without valid access keys.")

    // Generate access keys for each user
    for (const access of this.options.access) {
      const permissions = WeebPanelPermissionsBitField.resolve(access.permissions)
    ...
    if (this.options.logAccessKeys) {
      ...
      for (const [key, { permissions, userId }] of this.#generatedAccessKeys.entries()) {
        Logger.info(` UserID: ${userId}, Key: ${key}, Permissions: ${permissions}`)
      }
    }
  }
  ...
  public checkAccessKey(c: Context | string): { userId: string; token: string; permissions: number } | null {
